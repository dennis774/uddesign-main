<x-app-layout>
    ...
    <div class="container">
        ...
        <form action="{{ route('expense_details.update', $expense_detail->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select name="expense_type_id" id="expense_type_id" class="form-control" required>
                    @foreach($expense_types as $expense_type)
                        <option value="{{ $expense_type->id }}" {{ $expense_type->id == $expense_detail->expense_type_id ? 'selected' : '' }}>
                            {{ $expense_type->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="price" id="price" class="form-control" value="{{ $expense_detail->price }}" required>
            </div>
